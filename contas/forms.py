...
class CadastrarForm(ModelForm):
    nome = forms.CharField(max_length=60, widget=forms.TextInput(attrs={'onblur': 'validar(event)',
                                                                        'required': 'true',
                                                                                    'class': 'form-control'}))
    email = forms.CharField(max_length=200, widget=forms.TextInput(attrs={'onblur': 'validar(event)',
                                                                          'required': 'true',
                                                                          'class': 'form-control'}))
    cpf = forms.CharField(label='CPF', max_length=14, widget=forms.TextInput(attrs={'onkeydown': "mascara(this,icpf)", 'onblur': 'validar(event)',
                                                                                    'required': 'true',
                                                                                    'class': 'form-control'}))
    celular = forms.CharField(label="Celular", max_length=15, widget=forms.TextInput(
        attrs={'onkeydown': "mascara(this,icelular)", 'onload': 'mascara(this,icelular)', 'onblur': 'validar(event)',
               'required': 'true',
               'class': 'form-control'}))
    telefone = forms.CharField(label="Telefone", required=False, max_length=14, widget=forms.TextInput(
        attrs={'onkeydown': "mascara(this,itelefone)", 'onload': 'mascara(this,itelefone)', 'onblur': 'validar(event)',
               'required': 'true',
               'class': 'form-control'}))
    estado = forms.ModelChoiceField(queryset=Estado.objects.all(
    ), widget=forms.Select(attrs={'class': "selEstado"}))

    senha = forms.CharField(label='Senha:', widget=forms.PasswordInput)
    senha_confirma = forms.CharField(
        label='Confirmação de senha:', widget=forms.PasswordInput)

    field_order = ['nome', 'cpf', 'email',
                   'celular', 'telefone', 'estado', 'cidade']
...
class CadastroForm(ModelForm):
    ...
    telefone = forms.CharField(label="Telefone", required=False, max_length=14, widget=forms.TextInput(
        attrs={'onkeydown': "mascara(this,itelefone)", 'onload': 'mascara(this,itelefone)'}))
    estado = forms.ModelChoiceField(queryset=Estado.objects.all(
    ), widget=forms.Select(attrs={'class': "selEstado"}))
